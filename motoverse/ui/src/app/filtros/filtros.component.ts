...
@Component({
  ...
  template: `
    <div class="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-[100]" *ngIf="isOpen">
      <div class="bg-[var(--color-cinco)] p-6 rounded-lg w-[90%] max-w-md shadow-xl">
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-xl font-bold text-[var(--color-um)]">Filtros</h2>
          <button (click)="fecharFiltros()" class="text-[var(--color-um)] hover:opacity-80 transition-opacity cursor-pointer">
        ...
        <div class="flex justify-end gap-2 mt-6">
          <button (click)="limparFiltros()" 
                  class="px-4 py-2 rounded bg-gray-200 text-[var(--color-um)] hover:bg-gray-300 transition-colors cursor-pointer">
            Limpar
          </button>
          <button (click)="aplicarFiltros()" 
                  class="px-4 py-2 rounded bg-[var(--color-um)] text-white hover:opacity-90 transition-opacity cursor-pointer">
            Aplicar
          </button>
        </div>
      </div>
